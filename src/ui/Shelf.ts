 ...
 * rescaled to any number of rows and columns, to match the game grid size.游戏过程中的书架，作为三消游戏碎片的背景。它可以动态地调整为任意数量的行和列，以匹配游戏网格的大小。
 */
export class Shelf extends Container {
    ...
    public setup(options: { rows: number; columns: number; tileSize: number }) {
        ...
        const sizeDiff = 0.5; // Covers a small gap between blocks depending on the resolution根据分辨率覆盖块之间的小间隙。
