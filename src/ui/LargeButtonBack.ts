...
export class LargeButtonBack extends FancyButton {
    private label: Label;

    constructor(options: Partial<LargeButtonOptions> = {}) {
        const opts = { ...defaultLargeButtonOptions, ...options };

        const defaultView = new NineSlicePlane(Texture.from('button-large-back'), 36, 42, 36, 52);
        ...
        const pressedView = new NineSlicePlane(Texture.from('button-large-back-press'), 36, 62, 36, 52);
