...
export class LargeButtonConfirm extends FancyButton {
    private label: Label;

    constructor(options: Partial<LargeButtonOptions> = {}) {
        const opts = { ...defaultLargeButtonOptions, ...options };

        const defaultView = new NineSlicePlane(Texture.from('button-large-confirm'), 36, 42, 36, 52);
        ...
        const pressedView = new NineSlicePlane(
            Texture.from('button-large-confirm-press'),
            36,
            42,
            36,
            52,
